...
const aliases={volume:['volume','vol','n volume','nr volume','numero volume','nº volume'],pedido:['pedido','ordem','pedido venda'],item:['item','item pedido'],cliente:['cliente','tratamento cliente','tratamento/cliente','nome cliente'],dataEmbalagem:['data','data embalagem','data de embalagem'],equipe:['equipe','equipe embalagem'],beneficiador:['beneficiador','cod beneficiador','código beneficiador','codigo do beneficiador'],ferramenta:['ferramenta','perfil','ferramenta perfil'],amarra:['amarra'],pecas:['pecas','peças','qtd pecas','quantidade peças','quantidade de peças'],pesoLiquido:['peso liquido','peso líquido','kg','peso','peso líquido kg'],comprimento:['comprimento','corte'],beneficiamento:['beneficiamento','cor','codigo descricao beneficiamento','código/descrição de beneficiamento'],entrega:['entrega']};
...
export function importarB08(existing,rows,now=new Date().toISOString()){
 const map=new Map(existing.map(v=>[v.volume,v])),resultado=[],stats={totalLidos:0,pintura:0,ignorados:0,novos:0,atualizados:0,conhecidos:0,vinculados:0};
...
export function extractWorkbookRows(workbook,xlsx){
 ...
 const failures=[];
 for(const sheetName of workbook.SheetNames){try{const matrix=xlsx.utils.sheet_to_json(workbook.Sheets[sheetName],{header:1,defval:'',raw:true,blankrows:true});const parsed=matrixToB08Rows(matrix);return{...parsed,sheetName};}catch(error){failures.push(`${sheetName}: ${error.message}`);}}
