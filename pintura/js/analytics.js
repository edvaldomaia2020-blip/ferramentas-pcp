...
export const filaPintura=volumes=>volumes.filter(v=>v.beneficiador==='00153'&&v.equipe==='A'&&!v.confirmadoAcesys);
...
export function indicadores(volumes,lotes){const all=volumes.filter(v=>v.beneficiador==='00153'&&v.equipe==='A'),sum=xs=>round(xs.reduce((s,v)=>s+Number(v.pesoLiquido||0),0)),lotKg=stage=>round(lotes.filter(l=>stage(l)).reduce((s,l)=>s+Number(l.kgAtual||0),0));return{programado:sum(all),fila:sum(filaPintura(volumes).filter(v=>!v.loteId)),recebido:lotKg(l=>l.etapaAtual==='recebido'),processo:lotKg(l=>l.etapaAtual==='processo'),finalizadoApp:lotKg(l=>l.etapaAtual==='finalizado'),confirmadoAcesys:sum(all.filter(v=>v.confirmadoAcesys))};}
